...
show sub-percent agreement at half-integer ladder rungs; these are compelling
empirical coincidences that motivate, but do not replace, the full 3D toroidal
calculations deferred to Paper~2.  Bogoliubov--de~Gennes analysis confirms that
the planar LogSE vortex carries no bound breathing mode, establishing the 3D
...
\begin{itemize}
  \item \textbf{Spin.}
        ...
        quantization), $h$ is Planck's constant, and $m_0$ is a reference mass scale
        (self-consistently $m_e$ for the electron).  Spin $s=\hbar/2$ follows when
        this is combined with the half-quantum topology of the ring; the detailed
        Finkelstein--Rubinstein argument (showing that $\pi_1$ of the configuration
        space is $\mathbb{Z}_2$ and that the torus lies in the fermionic sector) will
        be given in Paper~2.
...
of the configuration space, cf.\ Paper~2) and the orbital flow ($L_{\rm orb}$).
...
\begin{itemize}
  ...
  \item \textbf{Mass.}
        ...
        without any fitting will be given in Paper~2.
\end{itemize}
...
Both calculations are deferred to Paper~2.  The neutron model is offered as a
...
in the LogSE + chiral-shear theory and is deferred to Paper~2.
...
Coupling to the transverse chiral channel must bridge this gap; finding that
coupled mode in the full 3D problem with $\mathcal{L}+\mathcal{L}_\perp$ is
the central calculation for Paper~2.  Until then:
\begin{equation}
...
and $F\approx4.47$ (preliminary numerical estimate, to be derived analytically in Paper~2),
...
integral (proton $F$), all deferred to Paper~2.
...
identification only; detailed derivations appear in Paper~2.
...
secondary flow fields in the plenum --- derived in Paper~2 and shown there to
...
The following open problems define the programme for Paper~2.  Each is a
...
Until these computations are completed, the ladder assignments for the muon
and proton should be read as predictive hypotheses — specific claims that the
3D calculations will confirm the values $3/2$ and $N_Y\cdot F\approx13.44$.
The pion is on stronger footing: its two-winding topology is topologically forced,
and the remaining gap is the stability proof for $L=2R_e$.
...
quantifying the gap that must be bridged by chiral coupling.  This is
Paper~2's central calculation.  Sub-percent agreement with CODATA for the
pion ($0.32\%$), muon ($0.61\%$, phenomenological), and proton
(preliminary $<0.3\%$ with $N_Y\approx3.007$, $F$ pending) is demonstrated.
...
%------------------------------------------------------------------
\section{The $\alpha$-Harmonic Ladder as a Musical Scale}
\label{app:musical}
%------------------------------------------------------------------
